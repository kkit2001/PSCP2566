...
def OnSelected(event):
    ...
    #=====LabelsChangeWindow=====
    lbl_title = Label(FormTitle, text="Updating Contacts", font=('arial', 16), bg="#ffdaa7",  width = 300)
...
def AddNewWindow():
    ...
    CONTACT.set("")
    NewWindow = Toplevel()
    NewWindow.title("Add Newcontact List")
    width = 400
    height = 300
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
    ...
    #=====LabelsForNewCon=====
    lbl_title = Label(FormTitle, text="Adding New Contacts", font=('arial', 16), bg="#77d998",  width = 300)
    lbl_title.pack(fill=X)
    lbl_firstname = Label(ContactForm, text="Firstname", font=('arial', 14), bd=5)
    lbl_firstname.grid(row=0, sticky=W)
    lbl_lastname = Label(ContactForm, text="Lastname", font=('arial', 14), bd=5)
...
Top.pack(side=TOP)
Mid = Frame(root, width=500,  bg="#feebbd")
Mid.pack(side=TOP)
MidLeft = Frame(Mid, width=100)
MidLeft.pack(side=LEFT, pady=10)
MidLeftPadding = Frame(Mid, width=370, bg="#feebbd")
...
btn_add.pack()
btn_delete = Button(MidRight, text="DELETE", bg="#f34233", fg="#eefffc", font=('arial', 16), command=DeleteData)
